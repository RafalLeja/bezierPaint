...
class BezierPaint:
  ...
  def setup_navbar(self):
    ...
    self.file_menu.add_command(label="Save lines", command=self.write_lines)
    self.file_menu.add_command(label="Open lines", command=self.read_lines)
    ...
    self.edit_menu.add_command(label="Grid", command=self.add_grid)
    self.edit_menu.add_command(label="Add background", command=self.add_background)

    self.italic_menu = tk.Menu(self.edit_menu, tearoff=False)
    self.edit_menu.add_cascade(label="Italic", menu=self.italic_menu)
    self.italic_menu.add_command(label="->", command=lambda: self.italic(1))
    self.italic_menu.add_command(label="<-", command=lambda: self.italic(-1))

    self.edit_menu.add_command(label="Join Bezier curves", command=self.join_lines_widget)
    self.edit_menu.add_command(label="Clear canvas", command=self.clear_canvas)
  ...
  def setup_tools(self):
    ...
    self.line_name_label = ttk.Label(self.tool_frame, text="Curve name:")
    ...
    self.line_type_label = ttk.Label(self.tool_frame, text="Curve type:")
    ...
    self.new_line_button = ttk.Button(self.tool_frame, text="Add new Curve", command=self.add_new_line)
    self.new_line_button.pack(side=tk.TOP, padx=5, pady=5)

    self.lines_label = ttk.Label(self.tool_frame, text="Curves:")
    ...
    self.new_point_button = ttk.Button(self.tool_frame, text="Add point", command=self.select_new_point_tool)
    self.new_point_button.pack(side=tk.TOP, padx=5, pady=5)

    self.dp_point_button = ttk.Button(self.tool_frame, text="Duplicate point", command=self.select_dp_point_tool)
    self.dp_point_button.pack(side=tk.TOP, padx=5, pady=5)

    self.move_point_button = ttk.Button(self.tool_frame, text="Move point", command=self.select_move_point_tool)
    self.move_point_button.pack(side=tk.TOP, padx=5, pady=5)

    self.delete_point_button = ttk.Button(self.tool_frame, text="Delete point", command=self.select_delete_point_tool)
    self.delete_point_button.pack(side=tk.TOP, padx=5, pady=5)

    self.delete_line_button = ttk.Button(self.tool_frame, text="Delete curve", command=self.delete_line)
  ...
  def update_types_box(self):
    self.line_type_combobox.current(0 + (self.selected_line_type == "Spline"))
  ...
  def join_lines_widget(self):
    ...
    selectionWindow.title("Select the curves you want to join")
    selectionWindow.geometry("400x200")
    selectionWindow.resizable(False, False)
    selectionWindow.grab_set()

    Label = tk.Label(selectionWindow, text="Curves must have at least 3 points, and will join smoothly in the middle of their last points")
    ...
    startA = tk.Radiobutton(selectionWindow, text="Start", variable=lineA_start, value=0)
    frameLeft.add(startA)
    endA = tk.Radiobutton(selectionWindow, text="End", variable=lineA_start, value=1)
    frameLeft.add(endA)
    ...
    startB = tk.Radiobutton(selectionWindow, text="Start", variable=lineB_start, value=0)
    frameRight.add(startB)
    endB = tk.Radiobutton(selectionWindow, text="End", variable=lineB_start, value=1)
    frameRight.add(endB)
    ...
    close_button = tk.Button(selectionWindow, text="Close", command=selectionWindow.destroy)
    close_button.pack(side=tk.LEFT, padx=5, pady=1)

    ok_button = tk.Button(selectionWindow, text="Join", command=lambda: self.join_lines(selectionWindow, self.lines[linesA_combobox.current()], self.lines[linesB_combobox.current()], lineA_start.get(), lineB_start.get()))
